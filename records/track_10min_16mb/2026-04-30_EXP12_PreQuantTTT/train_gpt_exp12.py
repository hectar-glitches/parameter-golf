...
def pre_quant_adamw_ttt(h, device, val_data, base_model):
	log("Starting Pre-Quantization TTT (Full BF16 on Val Tokens)")
	base_model.tok_emb.requires_grad_(False)
	for i in range(2):
		base_model.blocks[i].requires_grad_(False)
	ttt_params = [p for p in base_model.parameters() if p.requires_grad]
	lr = float(os.environ.get('PRE_QUANT_LR', 5e-4))
	epochs = int(os.environ.get('PRE_QUANT_EPOCHS', 21))
	optimizer = torch.optim.AdamW(ttt_params, lr=lr)
	base_model.train()
...
def train_and_eval(h,device):
	...
	# === EXP12 INJECTION: PRE-QUANTIZATION TEST TIME TRAINING ===
	pre_quant_adamw_ttt(h, device, val_data, base_model)
	torch._dynamo.reset();compiled_post_ttt=torch.compile(base_model,dynamic=False,fullgraph=True);timed_eval('post-prequant-ttt',eval_val,h,device,val_data,compiled_post_ttt)
